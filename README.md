...
基础地址必须使用供应商给出的 API 地址，通常到 `/v1`，不要填官网首页。先试「读取模型列表」。若网关返回网页或非 JSON，界面会提示检查地址；若模型成功响应却没有正文，会显示输出额度或格式问题，不再把空响应当作成功。单次输出上限可在 256–8192 Token 间调整；部分推理模型会把额度用在不可见的推理中。GPT/o 系列在兼容 Chat Completions 接口发送 `max_completion_tokens`，其他模型沿用 `max_tokens`，具体支持仍以供应商为准。
