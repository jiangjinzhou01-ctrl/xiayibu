...
3. **自带密钥的 API**：在「兼容 API」中选择接口格式。OpenAI 兼容格式使用 `GET /models`、`POST /chat/completions`，可选 GPT 或供应商提供的兼容模型；Claude 原生格式使用 `GET /models`、`POST /messages`，包含 `anthropic-version` 和浏览器直连标记。分别保存两种格式的基础地址与模型名，密钥只留在当前页面内存中；切换格式后需要重新填写对应密钥。基础地址应填写到 `/v1`，例如 `https://api.anthropic.com/v1`。所选服务必须允许浏览器跨域 CORS；若账户策略或网关禁止跨域，可使用自己的受保护中转接口。API 可能收费，聊天接口本身不代表有联网搜索；ChatGPT/Claude 订阅均不能直接当作 API 配额。
